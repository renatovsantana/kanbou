...
- **Key Features**:
    ...
    - **Client Onboarding**: Comprehensive onboarding page with about (rich text), products, services, social credentials, insights (message system with notifications), competitors, hashtags/tags, and user access control per client.
    - **Rich Text Editor**: TipTap-based rich text editor used in Kanban card descriptions and onboarding about field.
    ...
    - **Kanban Default Columns**: Backlog → Fila → Desenvolvendo Design → Desenvolvendo Copy → Em Aprovação → Aprovados → Revisão → Reprovados → Agendados → Postados → Finalizados (defined in DEFAULT_KANBAN_COLUMNS).
    - **Fixed Kanban Columns**: "Agendados", "Postados", "Finalizados" are fixed columns that cannot be deleted or renamed (FIXED_KANBAN_COLUMNS). All protected columns (including approval columns) are defined in PROTECTED_KANBAN_COLUMNS.

...
- **Key Features**:
    ...
    - **Kanban Default Columns**: Fila → Desenvolvendo Design → Desenvolvendo Copy → Em Aprovação → Tráfego e RDS → Revisão → Aprovados → Reprovados → Agendamento → Agendados → Postados → Finalizados (defined in DEFAULT_KANBAN_COLUMNS). "Agendamento" is a staging column where cards wait to be scheduled; cards show an "Agendar" button with confirmation dialog that moves them to "Agendados" (confirmed scheduled). "Informações", "Próximos serviços" and "Fotos já usadas" were removed.
